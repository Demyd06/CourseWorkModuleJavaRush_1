    ...
    public Runner(String[] args) {
        ...
        try {
            if (options.getCommand() == Commands.ENCRYPT) {
                ...
                String encryptedContent = enc.Encrypter(content, options.getKey(), Language.ENGLISH);
            ...
            if (options.getCommand() == Commands.BRUTEFORCE) {
                ...
                String fileName = options.getFilePath().getFileName().toString();
                String newFileName = fileName.substring(0, fileName.length() - 4) + " [BRUTEFORCE] key = " + bruteforce.getBruteForceKey() + ".txt";
